...
OpenSky Standard REST is **4,000 credits/day per independent bucket** (states / flights / tracks). Spending states credits does not buy flights credits. There is **no Trino** on this account — REST is the whole v1 path. Flights endpoints are a **nightly UTC batch** ([OpenSky FAQ](https://opensky-network.org/about/faq)): collect **yesterday UTC** (and earlier), **not today**. There is no live `/flights/all`. Same-day origin/destination cannot appear in the journal no matter how often we poll. Production waits on OpenSky’s overnight inference with **one** systemd collect at **06:00 UTC** (host journald: that run already sees a full yesterday batch of thousands of FlightObjects). Do not collect earlier to shave latency — a 03:28 UTC fetch saw ~⅓ the usual slice size and can lock 12/12 on a thinner day. Watch `/states/all` is a different product (who is up now) and does not insert trips.
